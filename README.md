...
- **Quick access panel** — a floating button opens the PinIt panel; click any pin to instantly activate that asset's placement tool.
...
Open the panel with the PinIt button (top-right), pick or create a collection, and start pinning. Use the **+ Pin** button in the tool options when an asset is selected, and the funnel button to assign it to filters.
...
```
%LocalAppData%Low\Colossal Order\Cities Skylines II\Mods\PinIt\
```

(The `PinIt.dll` and `PinIt.mjs` must sit together in that folder.)
...
```
%LocalAppData%Low\Colossal Order\Cities Skylines II\ModsData\PinIt\favourites.json
```
...
cd PinIt/UI
npm install
npm run build      # outputs dist/PinIt.mjs
```

**Build the mod**

Open `PinIt.slnx` in Visual Studio and build, or run `dotnet build`. The build's `DeployUI` step copies the prebuilt `PinIt.mjs` into the deployed mod folder. For quick UI-only iteration, `npm run deploy` rebuilds and copies just the `.mjs` to your Mods folder.
...
│   └── PinItUISystem.cs          C# ↔ UI bindings
